...
MainMenuScene::MainMenuScene(InputManager& input) : Scene{MAINMENUSCENE}, inputmgr{input}{}
...
/*  //--------------REMOVE WHEN DONE--------------------------
  else if (inputmgr.isKeyPressed(G)) /// used for testing game over scene remove when done
  {
  ...
  *///----------------------------------------------------------------
  else if (inputmgr.isKeyPressed(ENTER))
  {
    ...
    {
      return GAMESCENEMULTI;  //---Change to multiplayerscene when its done
    }
    else
    {
      return EXITGAME;
  ...
  return MAINMENUSCENE; //Do nothing
}
